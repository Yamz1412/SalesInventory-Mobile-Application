            ...
            for (ProductEntity pe : pending) {
                if ("DELETE_PENDING".equals(pe.syncState)) {
                    if (pe.productId != null && !pe.productId.isEmpty()) {
                        Task<Void> deleteTask = firestore.collection(firestoreManager.getUserProductsPath()).document(pe.productId).delete();
                        try {
                ...
                if (pe.productId != null && !pe.productId.isEmpty()) {
                    Task<Void> t = firestore.collection(firestoreManager.getUserProductsPath()).document(pe.productId).set(doc);
                    try {
                        Tasks.await(t);
                    ...
                    Task<com.google.firebase.firestore.DocumentReference> t = firestore.collection(firestoreManager.getUserProductsPath()).add(doc);
                    try {
