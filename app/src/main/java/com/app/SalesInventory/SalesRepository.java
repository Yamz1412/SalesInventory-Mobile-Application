...
public class SalesRepository {
    ...
    private void loadOverallRevenue() {
        // This calculates the total revenue (Lifetime or Monthly based on logic)
        // For Dashboard compatibility, we'll map this to the Total Lifetime Revenue for now
        // or strictly Monthly if that's what your logic preferred.
        firestoreManager.getDb().collection(firestoreManager.getUserSalesPath())
                .addSnapshotListener(MetadataChanges.INCLUDE, (snapshot, error) -> {
                    if (error != null) {
                        Log.e(TAG, "Error loading revenue", error);
                        return;
                    }
                    double total = 0.0;
                    if (snapshot != null) {
                    ...
                    if (snapshot != null) {
                        ...
                        for (DocumentSnapshot document : snapshot.getDocuments()) {
                            try {
                                ...
                                Log.e(TAG, "Error deserializing sales: " + document.getId(), e);
                            }
                        }
                    ...
                    if (snapshot != null) {
                        for (DocumentSnapshot document : snapshot.getDocuments()) {
                            try {
                                ...
                                Log.e(TAG, "Error parsing sale", e);
                            }
                        }
                    }
    ...
    // Kept for structure, though loadOverallRevenue covers the live update
    private void loadMonthlySales() {
        // Implementation typically identical to loadOverallRevenue but filtered by date
        // For now, we rely on the main listener.
    }
    ...
    // --- RELOAD METHODS (Restored for DashboardRepository) ---
    ...
    public void reloadMonthlySales() {
        // In this implementation, we re-trigger the revenue listener
        loadOverallRevenue();
    }
    ...
    // --- GETTERS (Restored for DashboardRepository) ---
    ...
    public void addSale(Sales sale, OnSaleAddedListener listener) {
        ...
        // Uses OnSuccessListener to ensure data is strictly saved before UI updates
        ref.set(map)
    ...
    // --- MISSING METHOD RESTORED (for DeliveryReportActivity) ---
    ...
    private Sales createSalesFromMap(Map<String, Object> data) {
        ...
        if (data.containsKey("quantity")) {
            ...
            if (qty instanceof Number) sale.setQuantity(((Number) qty).intValue());
        }

        if (data.containsKey("price")) {
            Object price = data.get("price");
            if (price instanceof Number) sale.setPrice(((Number) price).doubleValue());
        }

        if (data.containsKey("totalPrice")) {
            Object totalPrice = data.get("totalPrice");
            if (totalPrice instanceof Number) sale.setTotalPrice(((Number) totalPrice).doubleValue());
        }
